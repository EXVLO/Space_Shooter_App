...
def update_max_score_hacking(username, new_score):
    """Update max_score for a given player."""
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
...
def update_account_creation_date(username, new_date):
    """Update the account creation date (acc_created) for a given player."""
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
...
def send_score_data(client_socket):
    """Get max_score and acc_created from players and send to client."""
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
